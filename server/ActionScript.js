...
function doGet(e) {
  ...
  if (charset == "Auto") {
    let content = UrlFetchApp.fetch(url, {'headers': {'cookie': cookie}}).getBlob()
    ...
    return ContentService
      .createTextOutput(
        UrlFetchApp.fetch(url, {'headers': {'cookie': cookie}}).getContentText(charset)
      )
      .setMimeType(ContentService.MimeType.TEXT)
  }
}